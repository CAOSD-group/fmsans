...
def main(fm_filepath: str, n_tasks: int = 1, current_task: int = 1, n_min: int = -1, n_max: int = -1, n_current: int = -1, t_min: int = -1, t_max: int = -1):
   ...
   # print(f'Reading FM model... {fm_filepath}')
    ...
    fmsans_model = PicassoFMToFMSans(fm, n_tasks=n_tasks, current_task=current_task, n_min=n_min,
                                     n_max=n_max, n_current=n_current, min_time=t_min, max_time=t_max).transform()
    ...
    # else:
    output_fmsans_filepath = f'R_1_{current_task}-{n_tasks}.json'
...
if __name__ == '__main__':
    ...
    parser.add_argument('n_tasks', type=int,  default=-
                        1, help='Number of tasks.')
    parser.add_argument('current_task', type=int,
                        default=-1, help='Current task.')
    parser.add_argument('fileDivision', type=str,
    ...
    main(args.feature_model, args.n_tasks, args.current_task,
         n_min, n_max, n_current, args.t_min, args.t_max)
