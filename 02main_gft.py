...
from fm_solver.operations import FMConfigurationsNumber, FMConfigurations, FMCoreFeatures, FMFullAnalysisGFT, FMFullAnalysisBDD, FMFullAnalysisSAT
...
def main(fm_filepath: str, n_cores: int, runs: int) -> None:
    ...
    for i in range(1, runs + 1):
        ...
        with timer.Timer(name=TIME_ANALYSIS, logger=None):
            result = FMFullAnalysisGFT().execute(fm).get_result()
        signal.alarm(0)
    ...
    n_configs = result[FMFullAnalysisGFT.CONFIGURATIONS_NUMBER]
    core_features = result[FMFullAnalysisGFT.CORE_FEATURES]
    dead_features = result[FMFullAnalysisGFT.DEAD_FEATURES]
