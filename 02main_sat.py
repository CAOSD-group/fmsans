...
def main(fm_filepath: str, n_cores: int, runs: int) -> None:
    ...
    for i in range(1, runs + 1):
        ...
        with timer.Timer(name=TIME_ANALYSIS, logger=None):
            result = FMFullAnalysisSAT().execute(sat_model).get_result()
        signal.alarm(0)
    ...
    n_configs = result[FMFullAnalysisSAT.CONFIGURATIONS_NUMBER]
    core_features = result[FMFullAnalysisSAT.CORE_FEATURES]
    dead_features = result[FMFullAnalysisSAT.DEAD_FEATURES]
